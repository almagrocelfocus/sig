...
declare variable $serviceHeader as element()? (:: schema-element(tns:serviceHeader)? ::) external;
declare variable $inbound as element(*)? external;
declare variable $messageID as xs:string external;

declare function local:func($header as element(soap-env:Header)?,
                            $serviceHeader as element()? (:: schema-element(tns:serviceHeader)? ::), 
                            $inbound as element(*)?, 
    	...
    	return 
	    	...
	    	<he:headerExtended>
	        	...
	        	<he:messageId>{$messageID}</he:messageId>
                        <he:requestId>{fn:data($serviceHeader/requestId)}</he:requestId>
                        {
                            if ($serviceHeader/correlationId)
                            then <he:correlationId>{fn:data($serviceHeader/correlationId)}</he:correlationId>
                            else ()
                        }
                        <he:source>{fn:data($serviceHeader/source)}</he:source>
...
local:func($header, $serviceHeader, $inbound, $messageID)
