...
declare function xf:serviceLogEnd($headerInner as element()*,
    ...
    as element(log:input) {
    	let $responseCodes := $headerExtended/he:responseCodes
    	return
			<log:input>
				...
				<log:level>
                                {
                                  if(not($responseCodes) or $responseCodes/he:responseCode[1]/text() = '0') then
                                ...
                                <log:dynamicKeys>
                                {
                                       ...
                                       for $pathDynamicKeyHeader in $headerExtended/he:attributeList[1]/he:attribute[he:name='LOG_KEY_ADAPTER_END']
                                        return
                                          if ($pathDynamicKeyHeader/he:value[1]/text() != $headerExtended/he:attributeList[1]/he:attribute[he:name='LOG_HIDE_ADAPTER_END']/he:value[1]/text()) then
				...
				<log:task>{if(not($responseCodes) or $responseCodes/he:responseCode[1]/text() = '0') then 'ADAPTER_END' else 'ADAPTER_ERROR'}</log:task>
				...
				<log:statusCode>{if(not($responseCodes)) then '0' else $responseCodes/he:responseCode[1]/text()}</log:statusCode>
				<log:statusMessage>{if(not($responseCodes)) then 'Success' else 
                                if (fn:string-length($responseCodes/he:responseMessage[1]/text()) > 254)
                                then
				...
				<log:description>Received target system response</log:description>
