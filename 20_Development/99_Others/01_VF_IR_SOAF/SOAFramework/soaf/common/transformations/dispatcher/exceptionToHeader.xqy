...
(:: pragma parameter="$route" type="xs:anyType" ::)
(:: pragma bea:global-element-return element="soap-env:Header" location="../../schemas/soap_v1.1.xsd" ::)

declare namespace xf = "http://tempuri.org/SOAFramework/soaf/common/transformation/dispatcher/exceptionToHeader/";
...
declare function xf:exceptionToHeader($header as element(soap-env:Header)?,
    ...
    as element()*{
    	...
    	then $header/*
			...
			$header/*,
			if($body/*:Fault) then ( 
				xf:DecodeWSError($header, $body, $body/*:Fault, $inbound, $route)
			) else if($fault and fn:namespace-uri($fault) = fn:namespace-uri(<soap-env:Fault/>)) then (
	    		xf:DecodeWSError($header, $body, $fault, $inbound, $route)
	    	(: else check for OSB generic fault :) 
	    	) else if($fault and fn:namespace-uri($fault) = fn:namespace-uri(<con:fault/>) ) then (
	    		xf:DecodeOSBError($header, $body, $fault, $inbound, $route)
	    	) else ( 
	    		(: Unknown error type, to be decoded :)
	    		xf:ResponseCodes('1','Unknown fault type','1','Unknown fault type', false())
	    	)
};
