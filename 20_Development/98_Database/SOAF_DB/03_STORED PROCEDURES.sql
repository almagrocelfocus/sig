...
create or replace PACKAGE SOAF_LOG_CLEANER AS 
   PROCEDURE CLEAN_LOG; 
END SOAF_LOG_CLEANER; 
/
create or replace PACKAGE BODY SOAF_LOG_CLEANER AS 
   PROCEDURE CLEAN_LOG IS
   BEGIN
    DELETE LOG_KEYS LK WHERE LK.ID_LOG IN (SELECT L.ID FROM LOG L WHERE L.TIMESTAMP < TRUNC(SYSDATE - 30));
    DELETE LOG L WHERE L.TIMESTAMP < TRUNC(SYSDATE - 30);
   END CLEAN_LOG;
END SOAF_LOG_CLEANER; 
/
--------------------------------------------------------
--  DDL for Function CALCULATE_DOMAIN_WEIGHT
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "CALCULATE_DOMAIN_WEIGHT" (ID IN NUMBER, DOMAIN IN VARCHAR2, CATEGORY IN VARCHAR2, TARGET IN VARCHAR2, VERSION IN VARCHAR2, USERNAME IN VARCHAR2) 
   RETURN NUMBER 
